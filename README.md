...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/reciclaapp_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/reciclaapp_db
...
## 📸 Crear MaterialReciclados con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un materialreciclado. Puedes:
- ✅ Crear el materialreciclado SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el MaterialReciclado sin imagen

```bash
POST /api/materialreciclado
Authorization: Bearer {{token}}
Content-Type: application/json

{
  "nombre": "MaterialReciclado de ejemplo",
  "descripcion": "Descripción del materialreciclado",
  "disponible": true
}
```
...
**💡 Nota:** Guarda el `_id` del materialreciclado creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al MaterialReciclado

```bash
POST /api/materialreciclado/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del materialreciclado creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload MaterialReciclado Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3017/api/materialreciclado/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu materialreciclado ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3017/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3017/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
TOKEN=$(curl -X POST http://localhost:3017/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"productor@example.com","password":"password123"}' \
  | jq -r '.access_token')

# 2. Crear MaterialReciclado
PRODUCTO_ID=$(curl -X POST http://localhost:3017/api/materialreciclado \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "MaterialReciclado de ejemplo",   "descripcion": "Descripción del materialreciclado",   "disponible": true }' \
  | jq -r '.data._id')

echo "MaterialReciclado creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3017/api/materialreciclado/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3017/uploads/`
...
```bash
curl -X GET http://localhost:3017/api/usuario-profile/me \
...
```bash
curl -X PUT http://localhost:3017/api/usuario-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `reciclaapp-api.postman_collection.json`
...
├── reciclaapp-api.postman_collection.json
└── package.json
```
